...
Node* combiningTwoSets(Node* firstElemA, Node* firstElemB)
{
	...
	if (!isEmpty(firstElemA))
	{
		while (firstElemA)
		{
			result = addToBegin(result, firstElemA->val);
			firstElemA = firstElemA->next;
		}
	}

	if (!isEmpty(firstElemB))
	{
		while (firstElemB)
		{
			result = addToBegin(result, firstElemB->val);
			firstElemB = firstElemB->next;
		}
	}
...
Node* intersectionTwoSets(Node* firstElemA, Node* firstElemB)
{
	...
	while (firstElemB) {
		if (isInSet(firstElemB, firstElemA->val)) {		
			addToBegin(result, firstElemA->val);
		}
		firstElemB = firstElemB->next;
	}
	return result;
...
Node* differenceTwoSets(Node* firstElemA, Node* firstElemB)
{
	...
	Node* current = firstElemA;
	while (current != nullptr) {

		if (!isInSet(firstElemB, current->val)) {
			result = addToBegin(result, current->val);
		}
		current = current->next;
	}
...
{
	return combiningTwoSets(differenceTwoSets(firstElemA, firstElemB), differenceTwoSets(firstElemB, firstElemA));
}